...
def overview():
    ...
    layout = [dcc.Graph('line-graph', figure=line_fig, config={'displayModeBar': False}, style={'display': 'flex', 'height': '80vh'}),
              dcc.Graph('pie-chart', figure=pie_fig, config={'displayModeBar': False}, style={'display': 'flex', 'height': '80vh'})]
    return layout
...
def account_view(operator_name):
    ...
    layout = html.Div(className='row',
                      ...
                      children=[dcc.Graph('map', figure=map_fig, config={'displayModeBar': False}),
                                html.Div(id='detail', children=dcc.Graph('rank-table', figure=rank_table, config={'displayModeBar': False}))])
    return layout
...
def create_map(operator_name):
    ...
    map_df['overall_sent'] = map_df[
                                 'overall_sent'] / max_overall_sent  # scale all overall sents down so that the largest
...
def create_rank_table(operator_name):
    ...
    layout = [dcc.Graph('rank-table', figure=rank_table, config={'displayModeBar': False}, style={'display': 'flex', 'height': '80vh'})]  # *
    return layout
...
def update_page(clickData, value):  # Gets called whenever a region on the map is clicked
    operator_name = value
    if clickData == None:
